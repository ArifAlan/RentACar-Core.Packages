...
namespace Core.Persistence.Paging
{
    public static class IQueryablePaginateExtensions
    {
        public static async Task<Paginate<T>> ToPaginateAsync<T>( this IQueryable<T> source, int index,int size,CancellationToken cancellationToken = default )
        {
            ...
            List<T> items = await source.Skip(index * size).Take(size).ToListAsync(cancellationToken).ConfigureAwait(false);//index*size kadar datayı atlayıp ilgiliyi take ile  veriyi getirir
            ...
            Paginate<T> list = new()
            {
                ...
                Items = items,
                Size = size,
                Pages = (int)Math.Ceiling(count / (double)size)//ceiling yukarı yuvarlama
            };
            return list;
